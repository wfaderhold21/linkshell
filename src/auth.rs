...
pub fn operator_caps() -> CapSet {
    use Capability::*;
    [SignalState, Query, AgentSend, FirePipe, ManagePipes,
     Broadcast, CreateSession, InjectInput].into_iter().collect()
}

/// Default for a spawned worker agent — can report and talk, cannot escalate.
pub fn worker_caps() -> CapSet {
    use Capability::*;
    [SignalState, Query, AgentSend, FirePipe].into_iter().collect()
}
