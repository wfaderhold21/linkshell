...
async fn reply_err(w: &mpsc::Sender<String>, id: Option<u64>, code: ErrorCode, message: &str) {
    let env = Envelope {
        id,
        msg: Message::Error { code, message: message.to_string() },
    };
...
async fn handshake(
    ...
    let Message::Hello { protocol, token, name, group } = &first.msg else {
...
async fn handle_stream(
    ...
    match read_limited_line(&mut reader, max_bytes, &mut line).await {
        ...
        Ok(ReadResult::OversizeDrained) => {
            reply_err(&agent_tx, None, ErrorCode::Oversize, "message exceeds max_ipc_message_bytes").await;
            return;
        }
        Ok(ReadResult::Line) => {}
        Err(_) => return,
    ...
    let Some((registered_id, caps)) = handshake(&first_env, transport, &tx, &agent_tx).await
    else {
        return;
    };
    ...
    if let Some(sid) = registered_id {
        let _ = tx
            .send(AppEvent::IpcAgentConnected { session_id: sid, agent_tx: agent_tx.clone() })
            .await;
    }
            ...
            Ok(ReadResult::Line) => {
                ...
                if let Some(cap) = protocol::required_cap(&env.msg) {
                    if !caps.contains(&cap) {
                        reply_err(&agent_tx, env.id, ErrorCode::Forbidden, "capability denied").await;
                        continue;
                    }
                }
...
async fn dispatch_msg(
    ...
    match env.msg {
        Message::State { state, detail, session_id: msg_sid, session_name } => {
            if msg_sid.is_none() {
                if let Some(name) = session_name {
                    let raw = build_state_legacy_json(&state, detail.as_deref(), None, Some(&name));
                    let _ = tx.send(AppEvent::IpcNamedAction { session_name: name, msg: raw }).await;
                    return;
                }
            }
            let target = msg_sid.or(registered_id);
            let Some(sid) = target else { return };
            let _ = tx.send(AppEvent::IpcStateOverride { session_id: sid, state: state.clone() }).await;
            if let Some(d) = detail {
                let _ = tx.send(AppEvent::SessionOutput { session_id: sid, line: format!("[{}]", d) }).await;
            }
        }
        Message::Tokens { input, output, cost, session_id: msg_sid, session_name } => {
            if msg_sid.is_none() {
                if let Some(name) = session_name {
            ...
            let _ = tx.send(AppEvent::IpcTokenUpdate {
                session_id: sid,
                stats: TokenStats { input_tokens: input, output_tokens: output, total_cost_usd: cost, context_tokens: 0 },
            }).await;
        }
        Message::Output { line, session_id: msg_sid, session_name } => {
            if msg_sid.is_none() {
                if let Some(name) = session_name {
            ...
            let target = msg_sid.or(registered_id);
            let Some(sid) = target else { return };
            let _ = tx.send(AppEvent::SessionOutput { session_id: sid, line }).await;
        }
        Message::AgentSend { dest, message, wait } => {
            ...
            let _ = tx.send(AppEvent::AgentDirectMessage {
                from_session_id: registered_id, dest_name: dest, message, reply_tx,
            }).await;
        ...
        Message::FirePipe { source, dest, source_group } => {
        ...
        Message::Broadcast { group, message } => {
            let _ = tx.send(AppEvent::IpcBroadcast { group, msg: message }).await;
        }
        Message::PipeAdd { source, dest, trigger, extract, prefix } => {
            let _ = tx.send(AppEvent::IpcPipeAdd { source, dest, trigger, extract, prefix }).await;
        }
...
fn build_state_legacy_json(
    ...
    if let Some(d) = detail { v["detail"] = d.into(); }
    if let Some(sid) = session_id { v["session_id"] = sid.into(); }
    if let Some(name) = session_name { v["session_name"] = name.into(); }
    v
}
...
mod tests {
    ...
    fn socket_path_replaces_pid_placeholder_and_preserves_literal_paths() {
        ...
        assert_eq!(socket_path(&cfg), format!("/tmp/linkshell-{}.sock", std::process::id()));
    ...
    async fn read_limited_line_reports_oversize_and_recovers_at_next_line() {
        ...
        let mut reader = tokio::io::BufReader::new(reader);
        let mut line = String::new();
        assert_eq!(read_limited_line(&mut reader, 3, &mut line).await.unwrap(), ReadResult::OversizeDrained);
        assert_eq!(line, "abc");
        line.clear();
        assert_eq!(read_limited_line(&mut reader, 3, &mut line).await.unwrap(), ReadResult::Line);
        assert_eq!(line, "ok\n");
    }
    ...
    async fn dispatch_msg_uses_registered_id_for_state_and_tokens() {
        ...
        let (writer, _writer_rx) = mpsc::channel(1);
        dispatch_msg(Envelope { id: None, msg: Message::State {
            state: SessionState::Running, detail: Some("busy".to_string()),
            session_id: None, session_name: None,
        }}, &tx, Some(9), &writer).await;
        dispatch_msg(Envelope { id: None, msg: Message::Tokens {
            input: 3, output: 4, cost: 1.5, session_id: None, session_name: None,
        }}, &tx, Some(9), &writer).await;
        match rx.recv().await.unwrap() {
    ...
    async fn dispatch_msg_routes_pipe_broadcast_and_group_fire_messages() {
        ...
        let (writer, _writer_rx) = mpsc::channel(1);
        dispatch_msg(Envelope { id: None, msg: Message::FirePipe {
            source: None, dest: None, source_group: Some("agents".to_string()),
        }}, &tx, None, &writer).await;
        dispatch_msg(Envelope { id: None, msg: Message::FirePipe {
            source: Some(1), dest: Some(2), source_group: None,
        }}, &tx, None, &writer).await;
        dispatch_msg(Envelope { id: None, msg: Message::Broadcast {
            group: "agents".to_string(), message: serde_json::json!({"hello": true}),
        }}, &tx, None, &writer).await;
        dispatch_msg(Envelope { id: None, msg: Message::PipeAdd {
            source: 1, dest: 2, trigger: "manual".to_string(),
            extract: "diff".to_string(), prefix: Some("p".to_string()),
        }}, &tx, None, &writer).await;
